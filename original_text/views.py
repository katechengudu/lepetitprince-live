    ...
    def get_queryset(self,*args,**kwargs):
        ...
        if q1 or q2 or q3:
            if q2 == 'all' and q3 == 'french':
                results_sentence = Sentence.objects.filter(chapter_title__book_title__id=self.kwargs.get('pk')).filter(content__icontains=q1).order_by('id')
                ...
                results_sentence = Sentence.objects.filter(chapter_title__book_title__id=self.kwargs.get('pk')).filter(translation_eng__icontains=q1).order_by('id')
                total_results_sentence = len(results_sentence)
                results = {'results_sentence': results_sentence,
                           'total_results_sentence':total_results_sentence,
                           'chapter_titles':chapter_titles,
                ...
                results_sentence = Sentence.objects.filter(chapter_title__book_title__id=self.kwargs.get('pk')).filter(content__icontains=q1,chapter_title__title_name=q2).order_by('id')
                total_results_sentence = len(results_sentence)
                results = {'results_sentence': results_sentence,
                           'total_results_sentence':total_results_sentence,
                           'chapter_titles':chapter_titles,
                ...
                results_sentence = Sentence.objects.filter(chapter_title__book_title__id=self.kwargs.get('pk')).filter(translation_eng__icontains=q1,chapter_title__title_name=q2).order_by('id')
                total_results_sentence = len(results_sentence)
                results = {'results_sentence': results_sentence,
                           'total_results_sentence':total_results_sentence,
                           'chapter_titles':chapter_titles,
